    ...
    if request.method == 'POST':
        try:
            ...
            if entity_type == 'student':
                existing_identifiers = set(Student.objects.values_list('student_id', flat=True))
            elif entity_type == 'faculty':
                existing_identifiers = set(Faculty.objects.values_list('faculty_id', flat=True))
            elif entity_type == 'room':
                existing_identifiers = set(Room.objects.values_list('room_id', flat=True))
            ...
            for row in reader:
                ...
                if len(row) != len(headers):
                    current_row_errors.append(f"Row has incorrect number of columns on line {line_num}. Expected {len(headers)}, got {len(row)}.")
                ...
                if entity_type == 'student':
                    required_fields = ['student_id', 'name', 'department', 'study_year', 'semester', 'email']
                    for field in required_fields:
                        if not row_data.get(field):
                            current_row_errors.append(f"Missing required data for '{field}' on line {line_num}.")
                    try:
                        if 'study_year' in row_data:
                            row_data['study_year'] = int(row_data['study_year'])
                        else:
                            current_row_errors.append(f"Missing 'study year' on line {line_num}.")
                    except (ValueError, TypeError):
                        current_row_errors.append(f"Invalid 'study year' value on line {line_num}. Must be an integer.")
                    if not (row_data.get('student_id') and len(row_data['student_id']) == 10 and row_data['student_id'].isdigit()):
                        current_row_errors.append(f"Invalid 'student id' format on line {line_num}. Must be a 10-digit number.")
                elif entity_type == 'faculty':
                    required_fields = ['faculty_id', 'name', 'department', 'email']
                    for field in required_fields:
                        if not row_data.get(field):
                            current_row_errors.append(f"Missing required data for '{field}' on line {line_num}.")
                    if not (row_data.get('faculty_id') and len(row_data['faculty_id']) == 4 and row_data['faculty_id'].isdigit()):
                        current_row_errors.append(f"Invalid 'faculty id' format on line {line_num}. Must be a 4-digit number.")
                elif entity_type == 'room':
                    ...
                    try:
                        if 'capacity' in row_data:
                            ...
                            current_row_errors.append(f"Missing 'room capacity' on line {line_num}.")
                    except (ValueError, TypeError):
                        current_row_errors.append(f"Invalid 'room capacity' value on line {line_num}. Must be an integer.")
                    for optional_int_field in ['rowscount', 'columnscount']:
                        ...
                        if val:
                            try:
                                ...
                                current_row_errors.append(f"Invalid '{optional_int_field}' value on line {line_num}. Must be an integer or empty.")
                        else:
                            row_data[optional_int_field] = None
                if current_row_errors:
                    issues_detected.extend(current_row_errors)
                    continue
                model_fields_for_tuple = [f.name for f in Model._meta.fields if f.name != 'id']
                temp_obj_data = {k: v for k, v in row_data.items() if k in model_fields_for_tuple}
                ...
                try:
                    current_row_tuple = Model(**temp_obj_data).get_unique_row_tuple()
                except Exception as e:
                    issues_detected.append(f"Error preparing row for duplicate check on line {line_num}: {e}")
                    continue
                if current_row_tuple in existing_full_rows:
                    issues_detected.append(f"Skipping duplicate row (all fields match existing record) on line {line_num}.")
                    continue
                ...
                if row_data.get(main_identifier_field) in existing_identifiers:
                    issues_detected.append(f"Skipping row with duplicate {main_identifier_field}: '{row_data.get(main_identifier_field)}' on line {line_num}.")
                    continue
